...
- **GSAP 3.15 + @gsap/react** — centraal geregistreerd in `app/lib/gsap.js` (één plek; exporteert `gsap`, `useGSAP`, `ScrollTrigger`). Gebruikt op: `/portfolio` (DesignShowcase: ScrollTrigger + SplitText), **homepage-hero** (entree-timeline + magnetische CTA's via `app/components/magnetic.js`) en **Features** (USP-kaarten reveal via `ScrollTrigger.batch()`). Homepage-portfolio-preview gebruikt géén GSAP (plain rAF). Conventie: `useGSAP({ scope })` + `gsap.matchMedia('(prefers-reduced-motion: no-preference)')`; verborgen begin-staat via een `*-anim { opacity:0 }`-CSS-baseline met reduced-motion-override + `<noscript>`-fallback (geen FOUC, degradeert netjes). Bron-bonusbundel lokaal in `Downloads\gsap-public` (alle plugins gratis sinds v3.13). _framer-motion is verwijderd; Motion (motion.dev) bewust (nog) niet toegevoegd — GSAP-only._
...
- **2026-06-22 (CRO-animaties, fase 1 — GSAP-only)** — Start van een conversie-/motion-traject. Nieuw: `app/lib/gsap.js` (centrale plugin-registratie) en `app/components/magnetic.js` (herbruikbare magnetische CTA-wrapper via `gsap.quickTo`; uit op touch + reduced-motion). **Hero** (`hero.js`) van server-component + CSS-`fadeUp` → **client + GSAP-entree-timeline** (gestaggerde reveal, `power3.out`), beide CTA's nu magnetisch. **Features** (`features.js`) → **client**; USP-kaarten onthullen gestaggerd via `ScrollTrigger.batch()` bij in-scroll, hover-lift behouden met `clearProps:'transform'`. Beide met `*-anim`-opacity-baseline + reduced-motion-override + `<noscript>`-fallback. CTA-**copy** bewust ongewijzigd gelaten (beslissing open: bv. "Ontwerp jouw binder"). `npm run build` 2× schoon, alle 19 routes prerenderen statisch. ⚠️ **Watchpoint:** hero-H1 (LCP) onthult nu na hydration i.p.v. via pure CSS — na deploy Vercel Speed Insights checken; bij LCP-regressie H1 terug op CSS-reveal.
- **2026-06-22 (CRO-animaties, fase 2 — scroll-reveals doorgetrokken)** — Reveal-logica gerefactord naar één herbruikbare hook `app/lib/useReveal.js` (`useRevealOnScroll(scopeRef, selector?, opts?)` — `ScrollTrigger.batch()`, `once`, `clearProps:'transform'`, reduced-motion-gated). Globale conventie: **`.reveal-item`-baseline** (`opacity:0` + reduced-motion-override) in `globals.css`, en **één globale `<noscript>`-fallback** in `app/[locale]/layout.js` (dekt alle reveal-items). **Features** omgebouwd naar de hook (lokale `.feat-anim`-CSS + noscript weg → `.reveal-item`). **ProductRange** + **HowItWorks** → **client**, kaarten/stappen krijgen `.reveal-item` + de hook. **Portfolio-preview**: alleen de **header** (`.pf-head`) reveal't via de hook; de rAF-swipe-strip is **bewust ongemoeid** gelaten. `npm run build` schoon + `npm run lint` schoon. **Open knopen:** CTA-copy definitief maken + wel/niet Motion (motion.dev) toevoegen (nu GSAP-only).
- **2026-06-23 (CRO-animaties, fase 3 — reveal-consistentie afgemaakt)** — **SendIn** (`sendin.js`) was als enige contentsectie nog server-component zónder reveal → **client** + `useRevealOnScroll`; de 3 stappen + 3 info-items krijgen `.reveal-item` (header + CTA blijven meteen zichtbaar). Daarmee reveal'en alle contentsecties consistent. **Bewuste keuze:** de magnetische CTA-pull blijft **exclusief voor de hero** (elke knop magnetic maken voelt gimmicky en verwatert het effect). Geverifieerd: server-HTML rendert `reveal-item` op alle 6 SendIn-items, `npm run lint` schoon. (De hydration-mismatch in de dev-log tijdens de edit was een HMR-artefact, geen bug.) **Open knopen** ongewijzigd: CTA-copy + wel/niet Motion.
- **2026-06-23 (e-mail: professioneler + anti-spam)** — `app/api/submit-order/route.js`: klantbevestiging (`buildConfirmation`) herschreven naar een **table-based, inline-styled HTML-mail** (Outlook-proof) met merk-look (accentbalk #F5B301, serif-wordmark, details-kaart, footer met site + socials, verborgen preheader) **+ plain-text-variant**. Beide mails (interne notificatie + klantbevestiging) sturen nu **multipart `text`+`html`** (betere spam-score). i18n NL/EN blijft inline in de route (bestaande conventie voor e-mailteksten). `npm run build` + `lint` schoon. ⚠️ **KERN van het spam-probleem zit NIET in de code:** `ORDER_FROM_EMAIL` staat (lokaal) op `onboarding@resend.dev` — Resends gedeelde sandbox-afzender, faalt SPF/DKIM/DMARC-alignment → spam. **Actie voor eigenaar (DNS/dashboard, niet door mij te doen):** domein `75drawss.com` verifiëren in Resend (DKIM/SPF/DMARC-records toevoegen), daarna `ORDER_FROM_EMAIL` → `75Drawss <noreply@75drawss.com>` in `.env.local` **én** Vercel-env (Production) + redeploy. `.env.example` bevat nu deze waarschuwing. Preview-HTML stond in scratchpad (`email-nl.html`/`email-en.html`).
- **2026-06-23 (e-mail: interne notificatie zelfde merk-styling)** — De **interne order-notificatie** (de mail met de één-klik "Offerte sturen naar klant"-knop) kreeg dezelfde merk-shell als de klantbevestiging: accentbalk #F5B301, serif-wordmark + eyebrow "Nieuwe aanvraag", details-kaart in hetzelfde palet (`#faf9f6`/`#ececea`), table-based bulletproof CTA-knop (geel, `<td bgcolor>`-wrap i.p.v. losse `<a>` zodat Outlook 'm vult), en een footer-regel ("Antwoorden gaat rechtstreeks naar de klant …"). `npm run build` + `lint` schoon. Preview in scratchpad (`email-internal.html`).
- **2026-06-23 (e-mail live — afzender omgezet + gedeployd)** — Domein `75drawss.com` is geverifieerd in Resend. Via Vercel CLI de **Production** env-var `ORDER_FROM_EMAIL` omgezet van `onboarding@resend.dev` → **`75Drawss <orders@75drawss.com>`** (`.env.local` ook bijgewerkt). **Alleen de e-mail-wijzigingen gecommit** (`route.js`, `.env.example`, `HANDOFF.md` — commit `e73e3eb`) en naar `main` gepusht; de animatie-bestanden (hero/features/productrange/howitworks/portfolio/sendin/globals/layout + `magnetic.js`/`app/lib/`) blijven **bewust ongecommit/lokaal** tot akkoord. Auto-deploy `lzfqapj1w` = **Ready**, www.75drawss.com = 200. **Nog te doen (eigenaar):** (1) **Preview** env-var zetten in dashboard — CLI 54.12.2 had een bug bij `env add … preview` (de juiste vorm is `vercel env add ORDER_FROM_EMAIL preview --value '…' --yes`, faalde toch); (2) **DMARC**-record `_dmarc.75drawss.com` `v=DMARC1; p=none; rua=mailto:…` (optioneel, maakt 't af); (3) **testen** via mail-tester.com of een echte testbestelling. **Open:** animaties nog deployen + CTA-copy + wel/niet Motion.
- **2026-06-23 (conversie-tracking + ad-pixels, consent-gated)** — Meetfundament gelegd. Nieuw: `app/lib/track.js` (`trackLead()` — vuurt cookieloze **Vercel Analytics**-event `order_request` áltijd, + `fbq('Lead')`/`ttq('SubmitForm')` alléén als de pixels geladen zijn), aangeroepen in `orderform.js` bij succesvolle submit (non-PII: `service` + `product`). `app/lib/consent.js` (localStorage `cookie-consent` = granted/denied + `consentchange`-event). `app/components/pixels.js` (laadt **Meta + TikTok** base-scripts via `next/script` **alleen na consent én als `NEXT_PUBLIC_META_PIXEL_ID`/`NEXT_PUBLIC_TIKTOK_PIXEL_ID` gezet zijn**; PageView op route-change). `app/components/consentbanner.js` (lichte NL/EN-banner, `useSyncExternalStore` i.p.v. setState-in-effect → lint-clean; z-index 1000 boven de sticky bar z-70). Beide gemonteerd in `layout.js` (banner binnen `NextIntlClientProvider`, Pixels erbuiten). i18n-namespace `consent` (NL+EN), `.env.example` met de twee pixel-env-vars + uitleg. **AVG:** pixels laden pas na "Accepteren"; Vercel-analytics is cookieloos en draait altijd. `npm run build` + `lint` schoon; consent-banner rendert server-side. **Privacyverklaring bijgewerkt** (`messages/nl.json` + `en.json`, `privacy`-namespace): "Cookies"-sectie herschreven naar "Cookies en tracking" (noodzakelijk / cookieloze Vercel-stats / consent-gated TikTok+Meta-pixels / toestemming intrekken), en Meta Platforms Ireland + TikTok Technology Limited toegevoegd aan de verwerkerslijst. **Nog te doen (eigenaar):** pixel-ID's invullen in `.env.local` + Vercel-env (Production/Preview); custom events evt. inschakelen in Vercel-plan. **Gecommit `8f7286a` + gedeployd** (deploy `mrwet9dn9` Ready, banner + privacy live).
- **2026-06-23 (CRO: trust-balk in de hero)** — Risk-reversal-signalen van onder uit het bestelformulier naar pal **onder de hero-CTA's** getild: "Gratis offerte, geen verplichting · Geen betaling vooraf · Reactie binnen 1–2 werkdagen" (checkmark-icoontjes, accent-kleur). Nieuw `.hero-trust` blok in `hero.js` (eigen `.hero-trust-item`-stijl, `hero-anim`-baseline, reveal toegevoegd aan de GSAP-timeline tussen actions en stats; `hero-actions` marge 3rem→1.5rem). i18n `hero.trust1/2/3` (NL+EN). Vult een gat: hero-stats waren *specs* (100%/14–21/NL), de ticker is categorie-marquee — geruststelling bij de knop ontbrak. `npm run build` + `lint` schoon, rendert NL+EN. Nog **niet gecommit/gedeployd** (lokaal reviewen).
